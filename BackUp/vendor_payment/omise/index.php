<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>TEST omise</title>
    <script src="https://cdn.omise.co/omise.js" charset="utf-8"></script>
</head>
<body>
    <!-- <form id="checkoutForm" method="POST" action="checkout.php?tp=credit_card">
    <script type="text/javascript" src="https://cdn.omise.co/omise.js"
            data-key="pkey_test_5o4c2snkqt35roxoj7b"
            data-image="https://www.pmintermart.com/images/PM-LOGO.jpg"
            data-frame-label="PM Intermart"
            data-amount="10000"
            data-currency="THB"
            data-default-payment-method="credit_card">
    </script>
    </form> -->

    <form id="checkoutForm" method="POST" action="checkout.php?tp=truemoney">
        <script type="text/javascript" src="https://cdn.omise.co/omise.js"
            data-key="pkey_test_5o4c2snkqt35roxoj7b"
            data-image="https://cdn.omise.co/assets/dashboard/images/omise-logo.png"
            data-frame-label="PM Intermart"
            data-amount="100"
            data-currency="THB"
            data-default-payment-method="truemoney">
        </script>
    </form>
    

    
</body>
</html>