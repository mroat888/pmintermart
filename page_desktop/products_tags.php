                ...
                <div class="col-12" style="min-height: 500px;">
                    ...
                    <div class="row">
                        <?php 
                                ...
                                /*$str_sku = "select product_sku.* from product_sku, product_name 
                                where product_sku.id_products_name = product_name.id and 
                                (product_sku.tags LIKE :param_tag ) and product_sku.is_drop = :param_is_drop";*/

                                // $result_sku = $conn->prepare($str_sku);
                                // $result_sku->execute($array_sku);
                                // while($record_sku = $result_sku->fetch(PDO::FETCH_ASSOC)){

                                $str_product_name = "select product_name.name as product_name_name, product_sku.*, product_sku.name as product_sku_name 
                                from product_sku, product_name 
                                where product_sku.id_products_name = product_name.id and 
                                (product_sku.tags LIKE :param_tag ) and 
                                product_sku.price > :param_price and 
                                product_sku.is_drop = :param_is_drop ";
                                ...
                                $result_product_name->execute($array_sku);
