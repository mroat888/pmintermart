<?php
	if($record_sku['name'] !=""){
		$record_sku_name = " - ".$record_sku['name'];
		
	}else{
		$record_sku_name = "";
	}
	$product_name = $record_product_name['name'].$record_sku_name;
    $product_code = $record_sku['sku_code'];
    
    $link_product_name = $record_product_name['name'].$record_sku_name."-".$record_sku['sku_code'];
    $permalink_product_name = $pagetitle->setLinkReplace($link_product_name);
    $permalink = URL."product/".$record_sku['id']."/".$permalink_product_name."/";
    ...
    if(($record_sku['full_price'] != 0) && ($record_sku['full_price'] > $record_sku['price'])){
        ...
        $full_price = "<s> ราคาปกติ &#3647;".number_format($record_sku['full_price'])."</s>&nbsp;&nbsp;
...
<div class="box-card">
	<div class="div-box-tag" style="position: absolute;">
		<?php
			if(($record_sku['full_price'] != 0) && ($record_sku['full_price'] > $record_sku['price'])){
				$persent_discount = ($save_price*100)/$record_sku['full_price'];
		?>
	...
	<div class="card-body">
		<a href="<?php echo $permalink; ?>">
			<?php
				...
				if(file_exists(FOLDER_SKU."/".$record_sku['sku_code']."__2.jpg")) {
					$second_img = URL_IMG.FOLDER_SKU."/".$record_sku['sku_code']."__2.jpg";
				}else{
					$second_img = URL_IMG.FOLDER_SKU."/".$record_sku['sku_code']."__1.jpg";
				}
			?>
